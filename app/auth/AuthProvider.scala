...
abstract class AbstractAuthProvider {
  ...
  def firstUserEver: Boolean = transactional(readOnly) { all[UserModel].length == 0 }
...
object LDAPAuthProvider extends AbstractAuthProvider {
  ...
  def authenticate(username: String, password: String): Option[UserModel] = {
    ...
    val bindPassword = play.api.Play.current.configuration.getString("auth.ldap.password").get //TODO: obfuscation support
    ...
    val connection: LDAPConnection = new LDAPConnection(sslUtil.createSSLSocketFactory(), host, port, bindDN, bindPassword);
