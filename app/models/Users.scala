...
case class User(
  ...
  def isSuperuser = status == SuperUser
}
...
object User {
  ...
  def createLDAPUser(
    ...
    fullName: Option[String] = None): User = transactional {
  ...
  def paginatedListQuery(page: Int = 0, pageSize: Int = 100, orderBy: Int = 1, filter: String = "*")(implicit ctx: TransactionalExecutionContext) = {
    val pagination =
      asyncPaginatedQuery {
        (u: User) =>
